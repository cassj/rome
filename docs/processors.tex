...
% (the processor thaws the code refs and uses them in a Data::FormValidator profile for checking the arguments)
...
% The job entry in the database has a note of the process and via that the processor it requires. The idea is that the job scheduler can instantiate a new processor object, load in the script defined in the job and call the \texttt{process\_script} method to actually run it. 
% If in the future a job scheduler is sending jobs to be processed on a different machine, it will presumably need to generate a wrapper script to do this, which will be sent with the process script to the remote machine. 

package ROME::Controller::Component::simple_affy;
...
ROME::Controller::Component::simple_affy - ROME Component Controller
...
### START PROCESS plot_qc ###

=item plot_qc
  passes the template for the plot_qc process to the view

=cut
sub plot_qc :Local{

  my ($self, $c) = @_;
  $c->stash->{template} = 'component/simple_affy/plot_qc';

}

#parameter validation for plot_qc_queue
sub _validate_plot_qc :Private{
  my ($self, $c) = @_;
  my $dfv_profile = {
		      required => [qw()],	
		      optional => [qw()],	
...
=item plot_qc_queue
  ...
  Ajax action to which the component/simple_affy/plot_qc  form submits

=cut

sub plot_qc_queue :Path('plot_qc/queue'){
    my ($self, $c) = @_;
    ...
    $c->stash->{ajax} = 1;
     ...
     if ($c->forward('_validate_plot_qc')){
    
       # get your process 
       my $process = $c->model('ROMEDB::Process')->find({
              name => "plot_qc",
              component_name => "simple_affy",
              component_version =>"0.0.1"
       });
       die "process Plot QC not found" unless $process;
...
### START PROCESS make_simpleaffy_qc ###

=item make_simpleaffy_qc
  passes the template for the make_simpleaffy_qc process to the view

=cut
sub make_simpleaffy_qc :Local{

  my ($self, $c) = @_;
  $c->stash->{template} = 'component/simple_affy/make_simpleaffy_qc';

}

#parameter validation for make_simpleaffy_qc_queue
sub _validate_make_simpleaffy_qc :Private{
  my ($self, $c) = @_;
  my $dfv_profile = {
...
=item make_simpleaffy_qc_queue

  Ajax action to which the component/simple_affy/make_simpleaffy_qc  form submits

=cut

sub make_simpleaffy_qc_queue :Path('make_simpleaffy_qc/queue'){
    my ($self, $c) = @_;
    ...
    $c->stash->{ajax} = 1;
     ...
     #check your form parameters
     if ($c->forward('_validate_make_simpleaffy_qc')){
    
       # get your process 
       my $process = $c->model('ROMEDB::Process')->find({
              name => "make_simpleaffy_qc",
              component_name => "simple_affy",
              component_version =>"0.0.1"
       });
       die "process Make SimpleAffy Data not found" unless $process;
...
### END PROCESS make_simpleaffy_qc ###

package ROME::Controller::Component::Rma;
...
ROME::Controller::Component::Rma - ROME Component Controller
...
=item rma
  passes the template for the rma process to the view

=cut
sub rma :Local{

  my ($self, $c) = @_;
  $c->stash->{template} = 'component/rma/rma';

}

#parameter validation for rma_queue
sub _validate_rma :Private{
  my ($self, $c) = @_;
  my $dfv_profile = {
...
=item rma_queue
  ...
  Ajax action to which the component/rma/rma  form submits

=cut

sub rma_queue :Path('rma/queue'){
    my ($self, $c) = @_;
    ...
    $c->stash->{ajax} = 1;
     ...
     if ($c->forward('_validate_rma')){
       ...
       die "process RMA not found" unless $process;

package ROME::Controller::Component::Gcrma;
...
ROME::Controller::Component::Gcrma - ROME Component Controller
...
=item gcrma
  passes the template for the gcrma process to the view

=cut
sub gcrma :Local{

  my ($self, $c) = @_;
  $c->stash->{template} = 'component/gcrma/gcrma';

}

#parameter validation for gcrma_queue
sub _validate_gcrma :Private{
  my ($self, $c) = @_;
  my $dfv_profile = {
...
=item gcrma_queue
  ...
  Ajax action to which the component/gcrma/gcrma  form submits

=cut

sub gcrma_queue :Path('gcrma/queue'){
    my ($self, $c) = @_;
    ...
    $c->stash->{ajax} = 1;
     ...
     if ($c->forward('_validate_gcrma')){
    
       # get your process 
       my $process = $c->model('ROMEDB::Process')->find({
              name => "gcrma",
              component_name => "gcrma",
              component_version =>"0.0.1"
       });
       die "process gcRMA not found" unless $process;

package ROME::Controller::Component::affy_distribution_plots;
...
ROME::Controller::Component::affy_distribution_plots - ROME Component Controller
...
### START PROCESS boxplot ###

=item boxplot
  passes the template for the boxplot process to the view

=cut
sub boxplot :Local{

  my ($self, $c) = @_;
  $c->stash->{template} = 'component/affy_distribution_plots/boxplot';

}

#parameter validation for boxplot_queue
sub _validate_boxplot :Private{
  my ($self, $c) = @_;
  my $dfv_profile = {
...
=item boxplot_queue
  ...
  Ajax action to which the component/affy_distribution_plots/boxplot  form submits

=cut

sub boxplot_queue :Path('boxplot/queue'){
    my ($self, $c) = @_;
    ...
    $c->stash->{ajax} = 1;
     ...
     if ($c->forward('_validate_boxplot')){
       ...
       die "process Boxplot not found" unless $process;

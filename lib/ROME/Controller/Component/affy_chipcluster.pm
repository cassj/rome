package ROME::Controller::Component::affy_chipcluster;
...
ROME::Controller::Component::affy_chipcluster - ROME Component Controller
...
### START PROCESS chipcluster ###

=item chipcluster
  passes the template for the chipcluster process to the view

=cut
sub chipcluster :Local{

  my ($self, $c) = @_;
  $c->stash->{template} = 'component/affy_chipcluster/chipcluster';

}

#parameter validation for chipcluster_queue
sub _validate_chipcluster :Private{
  my ($self, $c) = @_;
  my $dfv_profile = {
...
=item chipcluster_queue

  Ajax action to which the component/affy_chipcluster/chipcluster  form submits

=cut

sub chipcluster_queue :Path('chipcluster/queue'){
    my ($self, $c) = @_;
    ...
    $c->stash->{ajax} = 1;
     ...
     if ($c->forward('_validate_chipcluster')){
    
       # get your process 
       my $process = $c->model('ROMEDB::Process')->find({
              name => "chipcluster",
              component_name => "affy_chipcluster",
              component_version =>"0.0.1"
       });
       die "process Cluster Chips not found" unless $process;

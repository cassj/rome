package ROME::Controller::Component::Mas5;
...
ROME::Controller::Component::Mas5 - ROME Component Controller
...
=item mas5
  passes the template for the mas5 process to the view

=cut
sub mas5 :Local{

  my ($self, $c) = @_;
  $c->stash->{template} = 'component/mas5/mas5';

}

#parameter validation for mas5_queue
sub _validate_mas5 :Private{
  my ($self, $c) = @_;
  my $dfv_profile = {
...
=item mas5_queue
  ...
  Ajax action to which the component/mas5/mas5  form submits

=cut

sub mas5_queue :Path('mas5/queue'){
    my ($self, $c) = @_;
    ...
    $c->stash->{ajax} = 1;
     ...
     if ($c->forward('_validate_mas5')){
       ...
       die "process MAS5 not found" unless $process;
